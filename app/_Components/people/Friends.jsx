...
import image2 from "@/app/profile.jpg"; // Ensure this path is correct
...
function Friends() {
   ...
   const images = Array.from({ length: 12 }, () => image2);
   ...
   return (
      <>
         ...
         <div className={`grid lg:grid-cols-5 sm:grid-cols-4 grid-cols-2 w-full gap-4 p-4 ${isExpanded ? "hidden" : "block"}`}>
            {images.slice(0, numImagesToShow-1).map((img, i) => (
               <Avatar key={i} onClick={()=>{
                  router.push(`/people/${i+1}`)
               }} className="sm:h-24 sm:w-24 h-16 w-16 flex justify-center items-center">
                  <AvatarImage src={img.src} alt={`Friend ${i + 1}`} />
